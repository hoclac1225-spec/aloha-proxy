// onboarding.js
// File chứa các hàm hiển thị thông báo / loading và hàm startOnboarding an toàn
// Sử dụng: import { startOnboarding, showNotification, showLoading } from './onboarding.js';

import { callAPI } from "./api.js";
import { validateUser } from "./validation.js";

/**
 * Hiển thị notification nhỏ góc trên/trên body.
...
/**
 * Bắt đầu quy trình onboarding (gọi API) — hàm đã harden để tránh uncaught promise
 * @param {Object} userData
 * @param {string} endpointOrBase
 * @returns {Promise<Object>} result object { ok, status, data, error, ... }
 */
export async function startOnboarding(userData, endpointOrBase = "/api/onboard") {
  ...
  // resolve runtime base if present
  const runtimeBase =
  ...
  showLoading(true);
  try {
    // callAPI in try/catch riêng để normalize mọi lỗi
    let response;
    try {
      // optional: pass timeoutMs via third param opts if needed
